		...
		if (MemberInfo.isRegex(srcTargetSelector)) {
			...
			if (srcInfo.name.equals(dstInfo.name) && !Constant.UNMAP_NAMES.contains(srcInfo.name)) {
				if (srcInfo.owner.isEmpty()) {
					Logger.warn("@Inject, @Redirect or @Modify*", targets, data.className, srcInfo.name);
				} else {
					Logger.warn("@Inject, @Redirect or @Modify*", Collections.singletonList(CommonUtility.classDescToName(srcInfo.owner)),
